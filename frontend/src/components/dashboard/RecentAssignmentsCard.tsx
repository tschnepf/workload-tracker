  ...
  return (
    <Card className={`rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-[0_10px_28px_rgba(0,0,0,0.25)] flex flex-col min-h-[240px] overflow-hidden ${className ?? ''}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-xs text-[var(--muted)]">{filteredItems.length}</span>
      </div>

      <div className="mt-4">
        <SearchTokenBar
          id="recent-assignments-search"
          label="Search recent assignments"
          placeholder={searchTokens.length ? 'Add another filter...' : 'Search people, clients, projects (Enter)'}
          tokens={searchTokens}

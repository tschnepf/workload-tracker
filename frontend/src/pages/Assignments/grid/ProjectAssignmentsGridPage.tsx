...
import { Assignment, Deliverable, Person, Project } from '@/types/models';
...
import { classifyWorkloadTokenTerm, filterTextCompatibleTokens, normalizeWorkloadAliasTerm } from '@/utils/workloadSearch';
...
import AddPersonRow from '@/pages/Assignments/project/components/AddPersonRow';
import { usePersonAssignmentAdd } from '@/pages/Assignments/project/hooks/usePersonAssignmentAdd';
import { useProjectAssignmentsGridController } from '@/pages/Assignments/grid/hooks/useProjectAssignmentsGridController';
...
const ProjectAssignmentsGrid: React.FC = () => {
  ...
  const {
    ...
    workloadHintVisible,
  } = useProjectAssignmentsGridController();
  ...
  const {
    ...
    listDefaultSettings: (phase) => autoHoursSettingsApi.list(undefined, phase),
  ...
  const handleAssignmentRoleChange = async (personId: number, assignmentId: number, roleId: number | null, roleName: string | null) => {
    ...
    try {
      const updated = await updateAssignmentRoleAction({
        assignmentsApi,
        setPeople: setPeople as any,
        setAssignmentsData,
        assignmentsData,
  ...
  const addActionColumnWidth = 29;
  const autoHoursColumnWidth = 29;
  const gridTemplate = useMemo(() => {
    const count = Math.max(1, visibleWeeks.length);
    return `${clientColumnWidth}px ${projectColumnWidth}px ${addActionColumnWidth}px ${autoHoursColumnWidth}px repeat(${count}, 70px)`;
  }, [clientColumnWidth, projectColumnWidth, visibleWeeks.length, addActionColumnWidth, autoHoursColumnWidth]);

  const totalMinWidth = useMemo(
    () => clientColumnWidth + projectColumnWidth + addActionColumnWidth + autoHoursColumnWidth + (weeks.length * 70) + 20,
    [clientColumnWidth, projectColumnWidth, weeks.length, addActionColumnWidth, autoHoursColumnWidth]
  );
  ...
  const ProjectDetailsButton: React.FC<{ projectId: number }> = ({ projectId }) => {
  ...
  const ProjectDashboardButton: React.FC<{ projectId: number }> = ({ projectId }) => (
    <Link
  ...
  const ProjectGroupHeader: React.FC<{ project: ProjectWithAssignments }> = ({ project }) => (
    <div className="col-span-2 flex items-center">
      <button
        ...
        className="flex-1 min-w-0 flex items-center gap-2 pl-3 pr-2 py-1 text-left transition-colors duration-200 rounded-sm"
      >
      ...
      <div className="pr-0 flex items-center gap-2">
        ...
        {project.id ? (
          <ProjectDetailsButton projectId={project.id} />
        ) : null}
        {project.id ? (
          <ProjectDashboardButton projectId={project.id} />
        ) : null}
      </div>
    </div>
  );
  ...
  const renderProjectSection = (project: ProjectWithAssignments) => {
        ...
        <div className="p-0">
          ...
          >
            <div style={{ minWidth: totalMinWidth, paddingLeft: weekPaddingLeft, paddingRight: weekPaddingRight }}>
              {visibleProjects.map((project) => renderProjectSection(project))}
            </div>
          </div>

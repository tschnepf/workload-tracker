    ...
    *,
    ...
    for batch in client.fetch(updated_since=cursor):
        for row in batch:
            ...
            external_id = _coerce_external_id(row.get('clientId') or row.get('id'))
            if not external_id:
            ...
            if dry_run:
                ...
                continue
            created = _apply_and_upsert(rule.connection, external_id, row, mapping)
            if created:
                metrics['inserted'] += 1
            else:
                metrics['updated'] += 1
...
def _apply_and_upsert(connection: IntegrationConnection, external_id: str, payload: Dict[str, Any], mapping: List[dict]) -> bool:
    ...
    updated_on = _parse_datetime(payload.get('updatedOn'))
    if updated_on:
        updates['updated_on'] = updated_on
    obj, created = IntegrationClient.objects.update_or_create(

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        try:
            subprocess.run(["psql", "-v", "ON_ERROR_STOP=1", "-c", f"CREATE DATABASE \"{temp_db}\" TEMPLATE template0;"], env=env, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        try:
            ...
            if path.endswith(".pgcustom"):
                cmd = ["pg_restore", "--no-owner", "--no-privileges", "--if-exists", "--clean", "-j", "2", "-d", temp_db, path]
            ...
            c1 = subprocess.run(["python", "manage.py", "check"], env=dj_env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            if c1.returncode != 0:
                raise RuntimeError((c1.stderr or "")[:1000])
            c2 = subprocess.run(["python", "manage.py", "migrate", "--check"], env=dj_env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            if c2.returncode != 0:
            ...
            if not opts.get("keep_db"):
                try:
                    subprocess.run(["psql", "-v", "ON_ERROR_STOP=1", "-c", f"DROP DATABASE IF EXISTS \"{temp_db}\";"], env=env, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                except Exception:
                    pass

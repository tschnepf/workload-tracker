...
1. **PreDeliverableType Model**: Define types of pre-deliverable items
2. **PreDeliverableItem Model**: Generated items linked to main deliverables
3. **ProjectSettings Model**: Per-project customization of pre-deliverable rules
4. **GlobalSettings Model**: System-wide default pre-deliverable rules

### Frontend Components
1. **Settings Interface**: Configure default pre-deliverable rules
...
Fields:
- project: OneToOneField to Project model with on_delete=CASCADE and related_name='pre_deliverable_settings'
...
**Prompt for AI Agent:**
```
Create a new Django model called GlobalPreDeliverableSettings in a new 'settings' app (create the app first). This model stores system-wide default settings for pre-deliverable generation. The model should include:
...
2. Global settings (GlobalPreDeliverableSettings)
...
4. calculate_working_days_before(target_date, days_before) - Calculates the actual date considering working days (exclude weekends)
...
- Use the GlobalPreDeliverableSettings.get_effective_settings method to determine what to generate
...
5. Update the calendar endpoint to include pre-deliverable items in the calendar data with a distinct visual indicator

Add proper permissions, filtering, and pagination. Follow existing code patterns in the codebase for consistency. Include comprehensive API documentation and maintain the existing naming conventions (snake_case for backend).
```
...
3. API integration with new endpoints for:
   - GET /api/settings/pre-deliverable-defaults/
   - PUT /api/settings/pre-deliverable-defaults/
...
1. Update the calendar API endpoint to include pre-deliverable items in the response
...
2. API endpoint for personal pre-deliverable items:
   - GET /api/calendar/personal-pre-deliverables/
   - POST /api/calendar/personal-pre-deliverables/{id}/complete/
